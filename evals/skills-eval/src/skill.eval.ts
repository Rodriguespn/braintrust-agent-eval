...
import { toolUsageScorer, referenceFilesReadScorer, supabaseBestPracticesScorer, toolCallEfficiencyScorer, selfInflictedStrugglesScorer, formatTranscriptForJudge, type Output, type ToolCallInfo } from './scorer.js'
...
function buildToolCallInfos(events: TranscriptEvent[]): ToolCallInfo[] {
  ...
  for (const e of events) {
    if (e.type === 'tool_call' && e.tool) {
      ...
      // so scenarios can match them via commandPattern (e.g. 'execute_sql', 'get_advisors')
      ...
      if (matchIdx !== -1) {
        ...
        // Fallback: match the oldest unmatched tool_call that has no success yet
...
function buildTaskOutput(events: TranscriptEvent[], summary: TranscriptSummary | undefined, evalRunData: EvalRunData): Output {
  ...
  const transcript = events.length > 0 ? formatTranscriptForJudge(events) : undefined

  return { finishReason, referenceFilesRead, toolCalls, generatedFiles: evalRunData.generatedFiles ?? {}, transcript }
}
...
Eval('Agent Skills', {
  ...
  data: () =>
    activeScenarios.map(([name, cfg]) => ({
      ...
      expected: {
        ...cfg.expected,
        // Resolve relative paths to absolute so the scorer can read file contents.
        referenceFilesRead: cfg.expected.referenceFilesRead.map((p) =>
          join(SKILLS_DIR, p)
        ),
      },
      tags: cfg.tags,
  ...
  task: async (input) => {
    ...
    if (evalRunData.result.error) {
      throw new Error(`Agent execution failed: ${evalRunData.result.error}`)
    }
    ...
    return buildTaskOutput(transcript?.events ?? [], transcript?.summary, evalRunData)
  },
  scores: [toolUsageScorer, referenceFilesReadScorer, supabaseBestPracticesScorer, toolCallEfficiencyScorer, selfInflictedStrugglesScorer],
})
