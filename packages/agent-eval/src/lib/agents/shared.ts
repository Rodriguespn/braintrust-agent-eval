 ...
 * Returns the initial commit SHA to pass to captureGeneratedFiles.
 */
export async function initGitAndCommit(sandbox: AnySandbox): Promise<string> {
  ...
  const result = await sandbox.runShell('git rev-parse HEAD');
  return result.stdout.trim();
}

/**
 * Capture source files generated by the agent.
 * Diffs against the initial commit SHA so files the agent committed during the
 * run are captured correctly (diffing HEAD would miss them).
...
export async function captureGeneratedFiles(
  ...
  try {
    ...
    // Diff against the initial commit SHA so any commits the agent made during
    // the run are included (git diff HEAD would show nothing for committed files).
    const findResult = await sandbox.runShell(`git add . && git diff ${initialCommitSha} --name-status`);
