 ...
 */
export type SetupFunction = (sandbox: DockerSandboxManager) => Promise<void>;

/**
 * Teardown function that runs after the agent finishes (before sandbox destruction).
 * Receives a sandbox instance for cleanup (e.g. stopping sibling containers).
 */
export type TeardownFunction = (sandbox: DockerSandboxManager) => Promise<void>;
...
export interface ExperimentConfig {
  ...
  /** Teardown function that runs after agent finishes, before sandbox destruction. @default undefined */
 ...
 */
export const EXCLUDED_FILES = ['node_modules', '.git'] as const;
...
export interface EvalFixture {
  ...
  path: string;
  /** The prompt to run for this eval */
  prompt: string;
}
