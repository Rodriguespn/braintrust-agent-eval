  ...
  return JSON.stringify(
    {
      ...
      type: 'module',
      scripts: {
        eval: 'npx braintrust eval experiments/braintrust.eval.ts',
      },
      devDependencies: {
...
# Braintrust API key - required for running evals with Braintrust
...
/**
 * Get the Braintrust eval entry point template.
 */
function getBraintrustEval(projectName: string): string {
  return `import { createBraintrustEval } from '@supabase/agent-evals';
import { join, dirname } from 'path';
 ...
 * Exports a default check function called on the host with the agent's generated files.
 */
function getExampleEval(): string {
  return `/**
 * Check function for this eval.
 * Receives the files generated by the agent and returns true if they look correct.
 *
...
export default function check(files: Record<string, string>): boolean {
  ...
  return app.includes('Welcome, user!');
}
`;
}
...
function getTemplateFiles(projectName: string): TemplateFile[] {
  return [
    ...
    { path: 'experiments/braintrust.eval.ts', content: getBraintrustEval(projectName) },
