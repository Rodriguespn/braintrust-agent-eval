 ...
 * Uses /tmp so Docker Desktop shares it with the host, enabling sibling
 * containers (e.g. from `supabase start`) to bind-mount workspace paths.
 */
...
export class DockerSandboxManager implements Sandbox {
  ...
  constructor(options: DockerSandboxOptions = {}) {
    ...
    // Each sandbox gets a unique workspace under /tmp so Docker Desktop
    // shares it with the host, enabling bind mounts from sibling containers.
  ...
  private async initialize(): Promise<void> {
    ...
    // Bind-mount the workspace directory so it's shared between this container,
    // the Docker host, and any sibling containers (Docker-out-of-Docker).
    this.container = await this.docker.createContainer({
      ...
      HostConfig: {
        ...
        ...(this.sysctls && Object.keys(this.sysctls).length ? { Sysctls: this.sysctls } : {}),
    ...
    await this.runCommandAsRoot('chown', ['-R', `${SANDBOX_UID}:${SANDBOX_GID}`, this.workdir]);
  ...
  /**
   * Run a shell command as root (through bash).
   * Useful for setup tasks that require elevated privileges.
   */
  async runShellAsRoot(command: string): Promise<CommandResult> {
    return this.runCommandAsRoot('bash', ['-c', command]);
  }
  ...
  async uploadFiles(files: SandboxFile[]): Promise<void> {
    ...
    // Upload to container
    await this.container.putArchive(pack, { path: this.workdir });
  ...
  async stop(): Promise<void> {
    ...
    // Clean up the host-side workspace directory
    try {
